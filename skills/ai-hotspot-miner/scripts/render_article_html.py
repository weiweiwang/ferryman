...
def _validate_clean_source(markdown_text: str) -> None:
    ...
    first_body_line = nonempty_body[0]
    if re.fullmatch(r"\*\*.+\*\*", first_body_line) and (
        "信噪比" in first_body_line or re.search(r"\d{4}-\d{2}-\d{2}", first_body_line)
    ):
        raise ValueError(
            "Article Markdown must contain only '# final title' and final body. "
            "Remove publication metadata from the article source."
        )

    last_body_line = nonempty_body[-1]
    if re.fullmatch(r"[*_]+.+[*_]+", last_body_line) and "信噪比" in last_body_line:
        raise ValueError(
            "Article Markdown must contain only '# final title' and final body. "
            "Remove brand footer metadata from the article source."
        )
